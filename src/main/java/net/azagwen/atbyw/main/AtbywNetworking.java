...
public class AtbywNetworking {
    public static final Identifier TINTING_COLOR_PACKET = clientPacketId("tinting_table_color");
    public static final Identifier TINTING_MODE_PACKET = clientPacketId("tinting_table_mode");
    public static final Identifier TINTING_UPDATE_PACKET = serverPacketId("tinting_table_update");
    ...
    public static void init() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(TINTING_UPDATE_PACKET, (server, player, handler, buf, sender) -> {
            server.execute(() -> {
                if (player.currentScreenHandler instanceof TintingTableScreenHandler sc) {
                    sc.input.markDirty();
                }
